...
def load_track_embedding_matrix(
    ...
    valid = finite_rows(embeddings)
    if not valid.all():
        dropped = int((~valid).sum())
        print(f"Dropping {dropped} tracks with non-finite embeddings.")
...
def mean_user_vector(user_lookup: dict[str, np.ndarray]) -> np.ndarray | None:
    ...
    valid = finite_rows(matrix)
    if not valid.any():
        return None
    return matrix[valid].mean(axis=0)
...
def build_user_query_matrix(
    ...
    valid = finite_rows(query_matrix)

    for user_id, is_valid in zip(user_ids, valid):
        if not is_valid and user_status[user_id] == "ok":
            user_status[user_id] = "non_finite"

    return query_matrix, user_status


def topk_smallest_indices(distances: np.ndarray, k: int) -> np.ndarray:
...
def l1_topk_indices(
    ...
    for chunk_start in range(0, track_matrix.shape[0], track_chunk_size):
        chunk = track_matrix[chunk_start : chunk_start + track_chunk_size]
        distances = np.abs(query_batch[:, None, :] - chunk[None, :, :]).sum(axis=2)
        local_k = min(max_k, distances.shape[1])
        local_indices = topk_smallest_indices(distances, local_k)
        local_distances = np.take_along_axis(distances, local_indices, axis=1)
        local_indices = local_indices + chunk_start
        ...
        keep_indices = topk_smallest_indices(merged_distances, max_k)
        best_distances = np.take_along_axis(merged_distances, keep_indices, axis=1)
...
def retrieve_topk_by_user(
    ...
    usable_indices = [
        ...
        if user_status[user_id] not in {"missing", "non_finite"}
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate CF-BPR raw L1-distance top-500 retrieval recall."
    )
    parser.add_argument("--tid", default="exp003_cf_bpr_l1_top500")
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Number of users scored at once for chunked L1 retrieval.",
    )
    parser.add_argument(
        "--track_chunk_size",
        type=int,
        default=4096,
        help="Number of tracks scored at once for chunked L1 retrieval.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    scores["metadata"] = {
        ...
        "retrieval_metric": "l1_distance",
        "embedding_normalization": "raw",
        "top_k": args.top_k,
        "k_values": k_values,

...
def load_or_train_item_vectors(
    ...
    if model_path.exists() and not force_retrain:
        print(f"Loading CountVectorizer + NMF model from {model_path}")
        ...
        if bundle.get("model_config") != model_config:
            raise ValueError(
                "Saved model settings do not match the requested settings. "
                "Use a different --model_path or --force_retrain."
            )
        return np.asarray(bundle["item_vectors"], dtype=np.float32), {
            ...
            "reconstruction_error": float(bundle["nmf"].reconstruction_err_),
            "nmf_n_iter": int(bundle["nmf"].n_iter_),
        }
    ...
    max_components = min(item_matrix.shape)
    if model_config["n_components"] > max_components:
        raise ValueError(
            f"--n_components must be <= {max_components} for item matrix "
            f"shape {item_matrix.shape}."
        )
    if (
        model_config["solver"] == "cd"
        and model_config["beta_loss"] != "frobenius"
    ):
        raise ValueError(
            "--solver cd only supports --beta_loss frobenius."
        )
    ...
    nmf = NMF(
        ...
        beta_loss=model_config["beta_loss"],
        tol=model_config["tol"],
        max_iter=model_config["max_iter"],
        random_state=model_config["seed"],
        alpha_W=model_config["alpha_W"],
        alpha_H=model_config["alpha_H"],
        l1_ratio=model_config["l1_ratio"],
    )
    print(
        f"Fitting NMF on item matrix {item_matrix.shape} with "
        f"{model_config['n_components']} components..."
    )
    item_vectors = normalize_rows(
        nmf.fit_transform(item_matrix).astype(np.float32)
    )
    ...
    print(f"Saved CountVectorizer + NMF model to {model_path}")
    return item_vectors, {
        ...
        retrieval_entries.append(
            {
                ...
                "predicted_response": (
                    "metadata CountVectorizer NMF mean-history cosine retrieval"
                ),
            }
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "NMF, then retrieve by cosine similarity to mean history vectors."
        )
    )
    parser.add_argument(
        "--tid", default="exp009_metadata_nmf_history_top500"
    )
    parser.add_argument(
    ...
    parser.add_argument("--nmf_solver", choices=["cd", "mu"], default="cd")
    parser.add_argument("--nmf_beta_loss", default="frobenius")
    parser.add_argument("--nmf_max_iter", type=int, default=200)
    parser.add_argument("--nmf_tol", type=float, default=1e-4)
    parser.add_argument("--nmf_alpha_w", type=float, default=0.0)
    parser.add_argument("--nmf_alpha_h", type=float, default=0.0)
    parser.add_argument("--nmf_l1_ratio", type=float, default=0.0)
    parser.add_argument("--retrieval_batch_size", type=int, default=64)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument(
        "--model_path", default="exp/models/exp009/metadata_nmf.pkl"
    )
...
def main() -> None:
    ...
    scores["metadata"] = {
        ...
        "user_vector": "mean_of_prior_turn_item_nmf_vectors",

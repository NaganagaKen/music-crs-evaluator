...
def load_or_train_item_vectors(
    ...
    if model_path.exists() and not force_retrain:
        print(f"Loading CountVectorizer + LDA model from {model_path}")
    ...
    print(
        f"Fitting LDA on item matrix {item_matrix.shape} "
        f"with {n_components} topics..."
    )
    item_vectors = normalize_rows(
        lda.fit_transform(item_matrix).astype(np.float32)
    )
    ...
    print(f"Saved CountVectorizer + LDA model to {model_path}")
    return item_vectors, {
        ...
        retrieval_entries.append(
            {
                ...
                "predicted_response": (
                    "metadata CountVectorizer LDA mean-history cosine retrieval"
                ),
            }
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "LDA, then retrieve by cosine similarity to mean history vectors."
        )
    )
    parser.add_argument(
        "--tid", default="exp006_metadata_lda_history_top500"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--learning_method", choices=["batch", "online"], default="online"
    )
    parser.add_argument("--lda_batch_size", type=int, default=1024)
    parser.add_argument("--retrieval_batch_size", type=int, default=64)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument(
        "--model_path", default="exp/models/exp006/metadata_lda.pkl"
    )
...
def main() -> None:
    ...
    item_vectors, model_stats = load_or_train_item_vectors(
        ...
        batch_size=args.lda_batch_size,
        seed=args.seed,
    ...
    scores["metadata"] = {
        ...
        "model": "count_vectorizer_latent_dirichlet_allocation",
        "metadata_fields": [
            "track_name",
            "artist_name",
            "album_name",
        ...
        "user_vector": "mean_of_prior_turn_item_topic_vectors",
        ...
        "max_iter": args.max_iter,
        "learning_method": args.learning_method,
        "lda_batch_size": args.lda_batch_size,
        "retrieval_batch_size": args.retrieval_batch_size,
        "seed": args.seed,
